...
function onEventReceived({ detail: event }) {
...
function initializeDefaultState(config) {
    ...
    const defaultState = {
        lists: {
            "Stream Goals": {
                tasks: [
                    { text: "Example Streamer Task 1", completed: false },
                    { text: "Example Streamer Task 2", completed: false }
                ],
...
window.CST_API = {
    ...
    addTaskToList: (listName, taskText) => {
        if (State.addTask(listName, { text: taskText, completed: false })) {
        ...
        const mockEvent = {
            ...
            event: {
                data: {
                    ...
                    tags: { broadcaster: true } // Assume broadcaster for testing
                }
            }
        };
        onEventReceived({ detail: mockEvent });
    }
};